...
const main = async (token: Tokens) => {
    ...
    const tx = await vaultContract.addAcceptedToken(TOKEN_CONTRACT_ADDRESS)
    await tx.wait();

    console.log(`Added ${token} with address ${TOKEN_CONTRACT_ADDRESS} to the accepted tokens`);
